			...
			<div class="con_area">
				...
				<div class="con_section ">
					<h2 class="con_sectionTit">
						안전보건관리 > <span class="subtit">연간계획수립</span>
					</h2>
				</div>
				...
				<div class="tab_wrap mt20">
					<ul class="tab_base N6">
						<li class="On"><a href="/page/KW30000/KW30000U1210.do"><span>평가분야</span></a></li>
...
function getPage(page) {
	...
	$.ajax({
				url : "/page/KW30000/KW30000U1210selectList", //서비스 주소 
				...
				success : function(res) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						data += '<td class="plnMidClss">' + list[i].plnMidClss + '</td>';
						data += '<td class="ltlImplDd">' + list[i].ltlImplDd + '</td>';
						data += '<td class="plnSchDd">' + list[i].plnSchDd + '</td>';
						data += '<td><button id="search" class="btn_His2" onclick="window.open(\'/mypage/MY80000U3100.do\');">견적/예약요청</button></td>';
						data += '</tr>';
					}
					} else {
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U1210delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		html += '<td class="plnMidClss"><input type="text" value="'+$target.find('.plnMidClss').text()+'"></td>';
		html += '<td class="ltlImplDd"><input type="date" value="'+$target.find('.ltlImplDd').text()+'"></td>';
		html += '<td class="plnSchDd"><input type="date" value="'+$target.find('.plnSchDd').text()+'"></td>';
		html += '<td><button type="submit" id="search" class="btn_His2">견적/예약요청</button></td>';
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="plnMidClss"><input type="text"></td>';
		html += '<td class="ltlImplDd"><input type="date" name="date" class="w140"></td>';
		html += '<td class="plnSchDd"><input type="date" name="date" class="w140"></td>';
		html += '<td></td>';
		html += '</tr>';
	...
	$("#saveBtn").on("click", function() {
		...
		for (i = 0; i < $("input[name=agree1][value='new']:checked").length; i++) {
			...
			arr.push({
				...
				plnMidClss : $target.find('.plnMidClss input').val(),
				ltlImplDd : $target.find('.ltlImplDd input').val(),
				plnSchDd : $target.find('.plnSchDd input').val()
			});
		}
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U1210save.do", param);
			 getPage();
	     }
	});
