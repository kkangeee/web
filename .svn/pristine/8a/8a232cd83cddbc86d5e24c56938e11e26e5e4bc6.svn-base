        ...
        <!-- ▼ 정보입력 영역 -->
				<div class="tab_wrap mt20">
					...
					<ul class="tab_base tab_base_sub N5">
						...
						<li class="On"><a href="/page/KW30000/KW30000U1173.do"><span>자동제세동기관리</span></a></li>
...
function getPage(page) {
	...
	$.ajax({
				url : "/page/KW30000/KW30000U1173selectList", //서비스 주소 
				//async : false,
				...
				success : function(res) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						data += '<td><button type="button" id="searchHisList" data-seq="' + list[i].seq + '" data-bzRgstNo="' + list[i].bzRgstNo + '" data-nsptDd="' + list[i].nsptDd + '" data-nsptPrsnNm="' + list[i].nsptPrsnNm + '" data-nsptPos="' + list[i].nsptPos + '" OnClick="chkPopClick(this);" class="btn_His2">실시</button></td>';
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		html += '<td><button type="submit" id="search" class="btn_His2">선임종료</button></td>';
	...
	function chkPopClick(obj) {
		...
		winObj = window.open("/page/KW30000/KW30000U1173chkPop.do", "점검표", pop_settings);
		...
		//일단 mgtNo에 대한 값을 받아 오는게 없어서 위치를 넣음
