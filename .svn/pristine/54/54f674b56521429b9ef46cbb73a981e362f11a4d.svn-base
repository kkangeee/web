...
<!-- #location -->
    ...
    <div id="container">
        ...
        <div class="con_section ">
            <h2 class="con_sectionTit">산업재해관리 / <span class="subtit">아차사고 등록</span></h2>            
        </div>
...
function getPage(page) {
	...
	$.ajax({
				url : "/page/KW30000/KW30000U6200selectList", //서비스 주소 
				...
				success : function(res) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						data += '<td class="rtprNm">' + list[i].rtprNm + '</td>';
						data += '<td class="prcsNm">' + list[i].prcsNm + '</td>';
						data += '<td class="wrkNm">' + list[i].wrkNm + '</td>';
						data += '<td class="acdntDtl">' + list[i].acdntDtl + '</td>';
						data += '<td class="fctrAnl">' + list[i].fctrAnl + '</td>';
						data += '<td class="acdntActn">' + list[i].acdntActn + '</td>';
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U6200delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		html += '<td class="rtprNm"><input type="text" value="'+$target.find('.rtprNm').text()+'"></td>';
		html += '<td class="prcsNm"><input type="text" value="'+$target.find('.prcsNm').text()+'"></td>';
		html += '<td class="wrkNm"><input type="text" value="'+$target.find('.wrkNm').text()+'"></td>';
		html += '<td class="acdntDtl"><input type="text" value="'+$target.find('.acdntDtl').text()+'"></td>';
		html += '<td class="fctrAnl"><input type="text" value="'+$target.find('.fctrAnl').text()+'"></td>';
		html += '<td class="acdntActn"><input type="text" value="'+$target.find('.acdntActn').text()+'"></td>';
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="rtprNm"><input type="text"></td>';
		html += '<td class="prcsNm"><input type="text"></td>';
		html += '<td class="wrkNm"><input type="text"></td>';
		html += '<td class="acdntDtl"><input type="text"></td>';
		html += '<td class="fctrAnl"><input type="text"></td>';
		html += '<td class="acdntActn"><input type="text"></td>';
	...
	$("#saveBtn").on("click", function() {
		...
		for (i = 0; i < $("input[name=agree1][value='new']:checked").length; i++) {
			...
			arr.push({
				...
				rtprNm : $target.find('.rtprNm input').val(),
				prcsNm : $target.find('.prcsNm input').val(),
				wrkNm : $target.find('.wrkNm input').val(),
				acdntDtl : $target.find('.acdntDtl input').val(),
				fctrAnl : $target.find('.fctrAnl input').val(),
				acdntActn : $target.find('.acdntActn input').val(),
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U6200save.do", param);
			 getPage();
	     }
