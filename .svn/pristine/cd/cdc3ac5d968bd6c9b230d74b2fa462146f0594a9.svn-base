...
public class QnaController {
	...
	public String qnaView(@ModelAttribute("customerVO") CustomerVO customerVO, Model model, HttpServletRequest req)
		...
		model.addAttribute("resultView1", qnaService.qnaView(Integer.parseInt(req.getParameter("contentId"))));
	...
	public String noticeUpdate(CustomerVO customerVO, HttpSession session) throws Exception {
		...
		return "redirect:/customer/qna/view.do?contentId=" + contentId;
	}
	...
	@ResponseBody
	public ResponseEntity<Boolean> pwChk(@ModelAttribute CustomerVO customerVO, HttpServletRequest request) throws Exception{
		
		customerVO.setContentId(Integer.parseInt(request.getParameter("contentId").toString()));
		customerVO.setSecretPw(request.getParameter("secretPw"));
		
		return ResponseEntity.ok(qnaService.chkPw(customerVO));
	}
}
