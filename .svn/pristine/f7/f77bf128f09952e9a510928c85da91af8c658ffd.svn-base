...
<!-- #location -->
    ...
    <div id="container">
        ...
        <div class="con_section ">
            <h2 class="con_sectionTit">작업환경관리 / <span class="subtit">위험성평가 교육관리</span></h2>            
        </div>
...
function getPage(page) {
	...
	$.ajax({
				url : "/page/KW30000/KW30000U4800selectList", //서비스 주소 
				...
				success : function(res) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						data += '<td class="dept">' + list[i].dept + '</td>';
						data += '<td class="pstnCls">' + list[i].pstnCls + '</td>';
						data += '<td class="eduStrDd">' + list[i].eduStrDd + '</td>';
						data += '<td class="eduEndDd">' + list[i].eduEndDd+ '</td>';
						data += '<td><button>이수증</button></td>';
						data += '</tr>';
					}
					} else {
						data += "<tr>";
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U4800delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		html += '<td class="tgtPplNm"><input type="text" value="'+$target.find('.tgtPplNm').text()+'"></td>';
		html += '<td class="dept"><input type="text" value="'+$target.find('.dept').text()+'"></td>';
		html += '<td class="pstnCls"><input type="text" value="'+$target.find('.pstnCls').text()+'"></td>';
		html += '<td class="eduStrDd"><input type="text" value="'+$target.find('.eduStrDd').text()+'"></td>';
		html += '<td class="eduEndDd"><input type="text" value="'+$target.find('.eduEndDd').text()+'"></td>';
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="tgtPplNm"><input type="text"></td>';
		html += '<td class="dept"><input type="text"></td>';
		html += '<td class="pstnCls"><input type="text"></td>';
		html += '<td class="eduStrDd"><input type="text"></td>';
		html += '<td class="eduEndDd"><input type="text"></td>';
		html += '</tr>';
		$("tbody").prepend(html);
		$("#deleteBtn").hide();
		$("#updateBtn").hide();
	...
	$("#saveBtn").on("click", function() {
		...
		for (i = 0; i < $("input[name=agree1][value='new']:checked").length; i++) {
			...
			arr.push({
				...
				dept : $target.find('.dept input').val(),
				pstnCls : $target.find('.pstnCls input').val(),
				eduStrDd : $target.find('.eduStrDd input').val(),
				eduEndDd : $target.find('.eduEndDd input').val()
			});
		}
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U4800save.do", param);
			 getPage();
	     }
	});
