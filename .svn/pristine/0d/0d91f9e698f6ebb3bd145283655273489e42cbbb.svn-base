			...
			<div class="con_area">
				...
				<div class="tab_wrap mt20">
					...
					<ul class="tab_base tab_base_sub N5">
						...
						<li class="On"><a href="/page/KW30000/KW30000U1174.do"><span>건강관리실방문자
									관리</span></a></li>
						<li><a href="/page/KW30000/KW30000U1175.do"><span>병원의뢰및
									이송 현황</span></a></li>
					</ul>
				</div>
...
function getPage(page) {
	...
	$.ajax({
				url : "/page/KW30000/KW30000U1174selectList", //서비스 주소 
				...
				success : function(res) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						data += '<td class="vsitDd">' + list[i].vsitDd + '</td>';
						data += '<td class="vsitrNm">' + list[i].vsitrNm + '</td>';
						data += '<td class="unitCoNm">' + list[i].unitCoNm + '</td>';
						data += '<td class="prcsNm">' + list[i].prcsNm + '</td>';
						data += '<td class="vsitDtl">' + list[i].vsitDtl + '</td>';
						data += '<td class="mainSymp">' + list[i].mainSymp + '</td>';
						data += '<td class="cnsltDtl">' + list[i].cnsltDtl + '</td>';
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U1174delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		html += '<td class="vsitDd"><input type="date" value="'+$target.find('.vsitDd').text()+'"></td>';
		html += '<td class="vsitrNm"><input type="text" value="'+$target.find('.vsitrNm').text()+'"></td>';
		...
		html += '<td class="vsitDtl"><input type="text" value="'+$target.find('.vsitDtl').text()+'"></td>';
		html += '<td class="mainSymp"><input type="text" value="'+$target.find('.mainSymp').text()+'"></td>';
		html += '<td class="cnsltDtl"><input type="text" value="'+$target.find('.cnsltDtl').text()+'"></td>';
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="vsitDd"><input type="date" name="date" class="w140"></td>';
		html += '<td class="vsitrNm"><input type="text"></td>';
		html += '<td class="unitCoNm"><input type="text"></td>';
		html += '<td class="prcsNm"><input type="text"></td>';
		html += '<td class="vsitDtl"><input type="text"></td>';
		html += '<td class="mainSymp"><input type="text"></td>';
		html += '<td class="cnsltDtl"><input type="text"></td>';
	...
	$("#saveBtn").on("click", function() {
		...
		for (i = 0; i < $("input[name=agree1][value='new']:checked").length; i++) {
			...
			arr.push({
				...
				vsitDd : $target.find('.vsitDd input').val(),
				vsitrNm : $target.find('.vsitrNm input').val(),
				unitCoNm : $target.find('.unitCoNm input').val(),
				prcsNm : $target.find('.prcsNm input').val(),
				vsitDtl : $target.find('.vsitDtl input').val(),
				mainSymp : $target.find('.mainSymp input').val(),
				cnsltDtl : $target.find('.cnsltDtl input').val(),
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U1174save.do", param);
			 getPage();
	     }
	});
