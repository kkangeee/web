         ...
         <!-- ▼ 정보입력 영역 -->
				<div class="tab_wrap mt20">
					<ul class="tab_base N6">
						...
						<li class="On"><a href="/page/KW30000/KW30000U1250.do"><span>환경관리분야</span></a></li>
...
function getPage(page) {
	...
	$.ajax({
				url : "/page/KW30000/KW30000U1250selectList", //서비스 주소 
				...
				success : function(res) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						data += '<td class="ltlImplDd">' + list[i].ltlImplDd + '</td>';
						data += '<td class="expsLmitExsPrcs">' + list[i].expsLmitExsPrcs + '</td>';
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U1250delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		html += '<td class="ltlImplDd"><input type="date" value="'+$target.find('.ltlImplDd').text()+'"></td>';
		html += '<td class="expsLmitExsPrcs"><input type="text" value="'+$target.find('.expsLmitExsPrcs').text()+'"></td>';
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="ltlImplDd"><input type="date" name="date" class="w140"></td>';
		html += '<td class="expsLmitExsPrcs"><input type="text"></td>';
	...
	$("#saveBtn").on("click", function() {
		...
		for (i = 0; i < $("input[name=agree1][value='new']:checked").length; i++) {
			...
			arr.push({
				...
				ltlImplDd : $target.find('.ltlImplDd input').val(),
				expsLmitExsPrcs : $target.find('.expsLmitExsPrcs input').val(),
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U1250save.do", param);
			 getPage();
	     }
	});
