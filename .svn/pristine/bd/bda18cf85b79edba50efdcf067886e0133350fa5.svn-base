	...
	<div class="inner">
		<!-- #content section////////////////////////////// -->
			...
			<section id="content">
				...
				<!-- .board-view -->
				<form id="boardForm" action="${pageContext.request.contextPath}/customer/notice/update" method="POST" encType="multipart/form-data">
				...
				<div class="button-group bottom">
					...
					<button type="button" class="secondary" onclick="location.href='/customer/notice/view.do?contentId=${resultView1.contentId}'">취소</button>
				</div>
			</section>
		<!-- //////////////////////////////#content section -->
	...
	$(document).ready(function() {
			...
			for(var i = 0; i < fileName.length; i++){
				if(fileName[i] !== ''){
					...
					$('<span style="cursor:pointer;color:blue" onclick="delfile('+ i +')"><button type="button" class="btn_del2">X</button></span></li>').appendTo($li);
			...
			$("input[type=file]").change(function() {
				var fileInput = document.getElementById("input-file");
						...
						for (var i = 0; i < files.length; i++) {
							file += '<li style="float: left;margin:0px 25px 0px 0px" id="filesDelAdd'+i+'">' +files[i].name+'<span style="cursor:pointer;color:blue"  data-delId="'+files[i].lastModified+'" data-delNo="'+i+'" onclick="delAddFile(this)"><button type="button" class="btn_del2">X</button></sapn></li>'
						}
			...
			var imgFiles = "";
			  ...
			  for (var i = 0; i < articles.length; i++) {
			     ...
			     for (var j = 0; j < imgs.length; j++) {
			       var img = imgs[j];
			       imgFiles += '<span style="margin:0px 25px 0px 0px" id="imgDel'+j+'">' +decodeURI(img.title)+'</span>'; 
			     }
