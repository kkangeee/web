	...
	<div class="inner">
		...
		<section id="content">
			...
			<div class="button-group bottom">
				<button type="button" class="secondary" onclick="location.href='/customer/notice/notice.do'">목록</button>
			<c:if test="${sessionScope.roleAuthId eq 'ADMIN'}">
				<button class="primary" type="submit" onclick="location.href='/customer/notice/update.do/${resultView1.contentId}'">수정</button>
...
<script>
	...
	$(document).on('click', '#btn_delete', function(e) {
		if (confirm("정말 삭제하시겠습니까 ?") == true) {
			var url = "/customer/notice/delete/"+${resultView1.contentId};
			location.href = url;
		} else {
			return;
		}
	...
	//이전페이지
	function fn_prev() {	
			var url = "<c:url value='/customer/notice/view.do'/>?contentId="+ ${resultView1.preContentId};
			location.href = url;
	}
	//다음페이지
	function fn_next() {
			var url = "<c:url value='/customer/notice/view.do'/>?contentId="+ ${resultView1.nextContentId};
			location.href = url;
	}
