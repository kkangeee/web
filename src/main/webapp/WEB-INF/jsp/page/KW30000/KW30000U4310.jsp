...
<!-- #location -->
    ...
    <div id="container">
        ...
        <div class="tab_wrap mt20">
            <ul class="tab_base N2">
                        <li class="On"><a href="/page/KW30000/KW30000U4310.do"><span>국소배기장치 등록 및 자체관리</span></a></li>
                        <li><a href="/page/KW30000/KW30000U4320.do"><span>국소배기장치 안전검사</span></a></li>
            </ul>
        </div>        
  ...
  <section id="content">
    ...
    <!-- 테이블01 --> 
    <span class="board_Ttxt">측정 결과서에 들어가있으면 자동으로 연계 / 측정 대상 물질(49종) 50% 초과 시 국소배기 안전검사에 자동 연계 / 외부 검사결과 업로드 기능 </span>
      <!-- 항목선택 -->
...
function getPage(page) {
	...
	$.ajax({
				url : "/page/KW30000/KW30000U4310selectList", //서비스 주소 
				...
				success : function(res) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						data += '<td class="dvceKnd">' + list[i].dvceKnd + '</td>';
						data += '<td class="msurWndSpd">' + list[i].msurWndSpd + '</td>';
						data += '<td class="stddWndSpd">' + list[i].stddWndSpd+ '</td>';
						data += '<td class="wndSpdUnit">' + list[i].wndSpdUnit + '</td>';
						data += '<td class="jdg">' + list[i].jdg + '</td>';
						data += '<td><button>작성</button><button>업로드</button></td>';
						data += '</tr>';
					}
					} else {
						data += "<tr>";
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U4310delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		html += '<td class="dvceKnd"><input type="text" value="'+$target.find('.dvceKnd').text()+'"></td>';
		html += '<td class="msurWndSpd"><input type="text" value="'+$target.find('.msurWndSpd').text()+'"></td>';
		html += '<td class="stddWndSpd"><input type="text" value="'+$target.find('.stddWndSpd').text()+'"></td>';
		html += '<td class="wndSpdUnit"><input type="text" value="'+$target.find('.wndSpdUnit').text()+'"></td>';
		html += '<td class="jdg"><input type="text" value="'+$target.find('.jdg').text()+'"></td>';
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="dvceKnd"><input type="text"></td>';
		html += '<td class="msurWndSpd"><input type="text"></td>';
		html += '<td class="stddWndSpd"><input type="text"></td>';
		html += '<td class="wndSpdUnit"><input type="text"></td>';
		html += '<td class="jdg"><input type="text"><button onclick = "deleteRow(this)">X</button></td>';
		html += '</tr>';
		$("tbody").prepend(html);
		$("#deleteBtn").hide();
		$("#updateBtn").hide();
	...
	$("#saveBtn").on("click", function() {
		...
		for (i = 0; i < $("input[name=agree1][value='new']:checked").length; i++) {
			...
			arr.push({
				...
				dvceKnd : $target.find('.dvceKnd input').val(),
				msurWndSpd : $target.find('.msurWndSpd input').val(),
				stddWndSpd : $target.find('.stddWndSpd input').val(),
				wndSpdUnit : $target.find('.wndSpdUnit input').val(),
				jdg : $target.find('.jdg input').val()
			});
		}
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U4310save.do", param);
			 getPage();
	     }
	});
