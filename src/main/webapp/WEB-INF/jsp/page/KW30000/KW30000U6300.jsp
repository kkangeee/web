...
<!-- #location -->
    ...
    <div id="container">
        ...
        <div class="con_section ">
            <h2 class="con_sectionTit">산업재해관리 / <span class="subtit">무재해 등록</span></h2>            
        </div>
...
function getPage(page) {
	...
	$.ajax({
				url : "/page/KW30000/KW30000U6300selectList", //서비스 주소 
				...
				success : function(res) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						data += '<td class="strDd">' + list[i].strDd + '</td>';
						data += '<td class="chrgPrsnNm">' + list[i].chrgPrsnNm + '</td>';
						data += '<td class="prcsNm">' + list[i].prcsNm + '</td>';
						data += '<td class="oneTmsTgtTm">' + list[i].oneTmsTgtTm + '</td>';
						data += '<td class="oneTmsTgtDays">' + list[i].oneTmsTgtDays + '</td>';
						data += '<td class="oneTmsAchvExpctnDd">' + list[i].oneTmsAchvExpctnDd + '</td>';
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U6300delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		html += '<td class="strDd"><input type="text" value="'+$target.find('.strDd').text()+'"></td>';
		...
		html += '<td class="oneTmsTgtTm"><input type="text" value="'+$target.find('.oneTmsTgtTm').text()+'"></td>';
		html += '<td class="oneTmsTgtDays"><input type="text" value="'+$target.find('.oneTmsTgtDays').text()+'"></td>';
		html += '<td class="oneTmsAchvExpctnDd"><input type="text" value="'+$target.find('.oneTmsAchvExpctnDd').text()+'"></td>';
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="strDd"><input type="text"></td>';
		html += '<td class="chrgPrsnNm"><input type="text"></td>';
		html += '<td class="prcsNm"><input type="text"></td>';
		html += '<td class="oneTmsTgtTm"><input type="text"></td>';
		html += '<td class="oneTmsTgtDays"><input type="text"></td>';
		html += '<td class="oneTmsAchvExpctnDd"><input type="text"></td>';
		html += '<td><button>등록</button></td>';
		html += '</tr>';
	...
	$("#saveBtn").on("click", function() {
		...
		for (i = 0; i < $("input[name=agree1][value='new']:checked").length; i++) {
			...
			arr.push({
				...
				strDd : $target.find('.strDd input').val(),
				chrgPrsnNm : $target.find('.chrgPrsnNm input').val(),
				prcsNm : $target.find('.prcsNm input').val(),
				oneTmsTgtTm : $target.find('.oneTmsTgtTm input').val(),
				oneTmsTgtDays : $target.find('.oneTmsTgtDays input').val(),
				oneTmsAchvExpctnDd : $target.find('.oneTmsAchvExpctnDd input').val()
			});
		}
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U6300save.do", param);
			 getPage();
	     }
