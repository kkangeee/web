...
<!-- #location -->
       ...
       <!-- ▼ 정보입력 영역 -->
				<div class="tab_wrap mt20">
					<ul class="tab_base N6">
						...
						<li class="On"><a href="/page/KW30000/KW30000U1230.do"><span>교육분야</span></a></li>
					...
					<ul class="tab_base tab_base_sub N2">
						<li class="On"><a href="/page/KW30000/KW30000U1230.do"><span>정기안전보건교육</span></a></li>
						<li><a href="/page/KW30000/KW30000U1230_1.do"><span>기타 교육</span></a></li>
					</ul>
				</div> 
  ...
  <section id="content">
    ...
    <!-- 테이블01 -->
    <span class="board-list_Stit">정기안전보건교육</span>
...
function getPage(page) {
	...
	var num = 1; 	
	$.ajax({
				url : "/page/KW30000/KW30000U1230selectList", //서비스 주소 
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U1230delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		if($target.find('.tp').text() == "정기"){
			html += '<td class="tp"><select style="width:80px" id="tp" class="select_bd_tit_A"><option value = "정기" selected>정기</option><option value = "수시">수시</option></select></td>';
		} else if($target.find('.tp').text() == "수시") {
			html += '<td class="tp"><select style="width:80px" id="tp" class="select_bd_tit_A"><option value = "정기">정기</option><option value = "수시" selected>수시ㄴ</option></select></td>';
		}
		html += '<td class="prid"><input type="text" value="'+$target.find('.prid').text()+'"></td>';
		html += '<td class="ttl"><input type="text" style="width: 400px" value="'+$target.find('.ttl').text()+'"></td>';
		html += '<td class="plnDtl"><input type="text" style="width: 250px"value="'+$target.find('.plnDtl').text()+'"></td>';
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="tp"><select style="width:80px" class="select_bd_tit_A"><option value="정기">정기</option><option value ="수시">수시</option></select></td>';
		html += '<td class="prid"><input type="text"></td>';
		html += '<td class="ttl"><input style="width: 400px" type="text"></td>';
		html += '<td class="plnDtl"><input style="width: 250px" type="text"></td>';
		html += '</tr>';
	...
	$("#saveBtn").on("click", function() {
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U1230save.do", param);
			 getPage();
	     }
	});
