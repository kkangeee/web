    ...
    <div id="container">
        ...
        <div class="tab_wrap mt20">
            <ul class="tab_base N2">
              <li class="On"><a href="/page/KW30000/KW30000U4610.do"><span>작업장 순회점검 등록</span></a></li>
              <li><a href="/page/KW30000/KW30000U4620.do"><span>작업장 순회점검 현황</span></a></li>
            </ul>
        </div>
...
function getPage(page) {
	...
	$.ajax({
				url : "/page/KW30000/KW30000U4610selectList", //서비스 주소 
				...
				success : function(res) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						data += '<td class="mprvNeedDtl">' + list[i].mprvNeedDtl + '</td>';
						data += '<td class="mprvActnDtl">' + list[i].mprvActnDtl+ '</td>';
						data += '<td class="actnLmt">' + list[i].actnLmt + '</td>';
						data += '<td class="othrDtl">' + list[i].othrDtl + '</td>';
						data += '</tr>';
					}
					} else {
						data += "<tr>";
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U4610delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		html += '<td class="mprvNeedDtl"><input type="text" value="'+$target.find('.mprvNeedDtl').text()+'"></td>';
		html += '<td class="mprvActnDtl"><input type="text" value="'+$target.find('.mprvActnDtl').text()+'"></td>';
		html += '<td class="actnLmt"><input type="text" value="'+$target.find('.actnLmt').text()+'"></td>';
		html += '<td class="othrDtl"><input type="text" value="'+$target.find('.othrDtl').text()+'"></td>';
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="mprvNeedDtl"><input type="text"></td>';
		html += '<td class="mprvActnDtl"><input type="text"></td>';
		html += '<td class="actnLmt"><input type="text"></td>';
		html += '<td class="othrDtl"><input type="text"><button onclick = "deleteRow(this)">X</button></td>';
		html += '</tr>';
		$("tbody").prepend(html);
		$("#deleteBtn").hide();
		$("#updateBtn").hide();
	...
	$("#saveBtn").on("click", function() {
		...
		for (i = 0; i < $("input[name=agree1][value='new']:checked").length; i++) {
			...
			arr.push({
				...
				mprvNeedDtl : $target.find('.mprvNeedDtl input').val(),
				mprvActnDtl : $target.find('.mprvActnDtl input').val(),
				actnLmt : $target.find('.actnLmt input').val(),
				othrDtl : $target.find('.othrDtl input').val()
			});
		}
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U4610save.do", param);
			 getPage();
	     }
	});
