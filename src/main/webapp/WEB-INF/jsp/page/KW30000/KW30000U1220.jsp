...
<!-- #location -->
        	...
        	<!-- ▼ 정보입력 영역 -->
				<div class="tab_wrap mt20">
					...
					<ul class="tab_base tab_base_sub N3">
						<li class="On"><a href="/page/KW30000/KW30000U1220.do"><span>산업안전보건위원회</span></a></li>
						<li><a href="/page/KW30000/KW30000U1220_1.do"><span>안전 및 보건에 관한 협의체</span></a></li>
  ...
  <section id="content">
    ...
    <!-- 테이블01 -->
    <span class="board-list_Stit">산업안전보건위원회</span>
...
function getPage(page) {
	...
	var num = 1; 	
	$.ajax({
				url : "/page/KW30000/KW30000U1220selectList", //서비스 주소 
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U1220delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		if($target.find('.prid').text() == "1분기"){
			html += '<td class="prid"><select style="width:80px" id="prid" class="select_bd_tit_A"><option value = "1분기" selected>1분기</option><option value = "2분기">2분기</option><option value = "3분기">3분기</option><option value = "4분기">4분기</option></select></td>';
		} else if($target.find('.prid').text() == "2분기") {
			html += '<td class="prid"><select style="width:80px" id="prid" class="select_bd_tit_A"><option value = "1분기">1분기</option><option value = "2분기" selected>2분기</option><option value = "3분기">3분기</option><option value = "4분기">4분기</option></select></td>';
		} else if($target.find('.prid').text() == "3분기") {
			html += '<td class="prid"><select style="width:80px" id="prid" class="select_bd_tit_A"><option value = "1분기">1분기</option><option value = "2분기">2분기</option><option value = "3분기" selected>3분기</option><option value = "4분기">4분기</option></select></td>';
		} else if($target.find('.prid').text() == "4분기") {
			html += '<td class="prid"><select style="width:80px" id="prid" class="select_bd_tit_A"><option value = "1분기">1분기</option><option value = "2분기">2분기</option><option value = "3분기">3분기</option><option value = "4분기" selected>4분기</option></select></td>';
		}
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="prid"><select style="width:80px" class="select_bd_tit_A"><option value="1분기">1분기</option><option value ="2분기">2분기</option><option value="3분기">3분기</option><option value ="4분기">4분기</option></select></td>';
	...
	$("#saveBtn").on("click", function() {
		...
		for (i = 0; i < $("input[name=agree1][value='new']:checked").length; i++) {
			...
			arr.push({
				...
				prid : $target.find('.prid select').val(),
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U1220save.do", param);
			 getPage();
	     }
	});
