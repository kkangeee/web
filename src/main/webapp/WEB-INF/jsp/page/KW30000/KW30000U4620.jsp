...
<!-- #location -->
    ...
    <div id="container">
        ...
        <div class="con_section ">
            <h2 class="con_sectionTit">작업환경관리 / <span class="subtit">작업장 순회점검</span></h2>            
        </div>
        
        <!-- ▼ 정보입력 영역 -->
        <div class="tab_wrap mt20">
            <ul class="tab_base N2">
              <li><a href="/page/KW30000/KW30000U4610.do"><span>작업장 순회점검 등록</span></a></li>
              <li class="On"><a href="/page/KW30000/KW30000U4620.do"><span>작업장 순회점검 현황</span></a></li>
            </ul>
        </div>
...
function getPage(page) {
	...
	$.ajax({
				url : "/page/KW30000/KW30000U4620selectList", //서비스 주소 
				...
				success : function(res) {
					...
					for (var i = 0; i < list.length; i++) {
						...
						data += '<td class="nsptDt">' + list[i].nsptDt + '</td>';
						data += '<td class="nsptPrsnNm">' + list[i].nsptPrsnNm + '</td>';
						data += '<td class="prcsNm">' + list[i].prcsNm + '</td>';
						data += '<td class="mprvNeedYn">' + list[i].mprvNeedYn + '</td>';
						data += '<td class="mprvActnYn">' + list[i].mprvActnYn+ '</td>';
						data += '<td class="actnLmt">' + list[i].actnLmt + '</td>';
						data += '<td><button type="submit" id="search" class="btn_His2">일지출력</button></td>';
						data += '<td><button type="submit" id="search" class="btn_His2">점검표</button></td>';
						data += '</tr>';
					}
					} else {
						data += "<tr>";
...
$("#deleteBtn").on("click",function(){
	...
	var param = {
	 ...
	 if (confirm("삭제하시겠습니까?")) {
		common.ajaxaa("/page/KW30000/KW30000U4620delete.do", param);
		 getPage();
     }
});
...
$("#updateBtn").on("click", function() {
	...
	for (i = 0; i < $("input[name=agree1]:checked").length; i++) {
		...
		html += '<td class="nsptDt"><input type="text" value="'+$target.find('.nsptDt').text()+'"></td>';
		...
		html += '<td class="mprvNeedYn"><input type="text" value="'+$target.find('.mprvNeedYn').text()+'"></td>';
		html += '<td class="mprvActnYn"><input type="text" value="'+$target.find('.mprvActnYn').text()+'"></td>';
		html += '<td class="actnLmt"><input type="text" value="'+$target.find('.actnLmt').text()+'"></td>';
		html += '<td><button type="submit" id="search" class="btn_His2">일지출력</button></td>';
		html += '<td><button type="submit" id="search" class="btn_His2">점검표</button></td>';
	...
	$("#insertBtn").on("click",function() {
		...
		html += '<td class="nsptDt"><input type="text"></td>';
		html += '<td class="nsptPrsnNm"><input type="text"></td>';
		html += '<td class="prcsNm"><input type="text"></td>';
		html += '<td class="mprvNeedYn"><input type="text"></td>';
		html += '<td class="mprvActnYn"><input type="text"></td>';
		html += '<td class="actnLmt"><input type="text"></td>';
		html += '<td><button type="submit" id="search" class="btn_His2">일지출력</button></td>';
		html += '<td><button type="submit" id="search" class="btn_His2">점검표</button><button onclick = "deleteRow(this)">X</button></td>';
		html += '</tr>';
		$("tbody").prepend(html);
		$("#deleteBtn").hide();
		$("#updateBtn").hide();
	...
	$("#saveBtn").on("click", function() {
		...
		for (i = 0; i < $("input[name=agree1][value='new']:checked").length; i++) {
			...
			arr.push({
				...
				nsptDt : $target.find('.nsptDt input').val(),
				nsptPrsnNm : $target.find('.nsptPrsnNm input').val(),
				prcsNm : $target.find('.prcsNm input').val(),
				mprvNeedYn : $target.find('.mprvNeedYn input').val(),
				mprvActnYn : $target.find('.mprvActnYn input').val(),
				actnLmt : $target.find('.actnLmt input').val(),
			});
		}
		...
		if (confirm("저장하시겠습니까?")) {
			common.ajaxaa("/page/KW30000/KW30000U4620save.do", param);
			 getPage();
	     }
	});
